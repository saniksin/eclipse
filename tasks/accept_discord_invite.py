...
from settings.settings import NUMBER_OF_ATTEMPTS, SITEKEY, CAPTCHA_SERVICE, API_KEY_24_CAPTCHA, API_KEY_BESTCAPTCHA, API_KEY_RAZORCAP
...
from utils.captcha.razorcap import create_razorcap_task, get_razorcap_task_result
    ...
    async def accept_invite(self, my_try=100):
        while True:
            ...
            if "You need to update your app to join this server." in response.text or "captcha_rqdata" in response.text:
                ...
                # with open('import/discord_proxys.txt', 'r') as file:
                ...
                #return await self.accept_invite(my_try + 1)
                #continue

                captcha_rqdata = response.json()["captcha_rqdata"]
                captcha_rqtoken = response.json()["captcha_rqtoken"]

                PAGE_URL = f"https://discord.com/"
                PROXY = self.proxy.replace('http://', '')
                PROXY_TYPE = 'HTTP'

                logger.info(f"[{self.acc_index}] | {self.discord_token} | Creating hCAPTCHA task...")
                if CAPTCHA_SERVICE == "CAPTCHA24":
                    status, task_id = await create_24captch_task(self.async_session, API_KEY_24_CAPTCHA, SITEKEY, PAGE_URL, PROXY, PROXY_TYPE, captcha_rqdata)
                elif CAPTCHA_SERVICE == "BESTCAPTCHA":
                ...
                elif CAPTCHA_SERVICE == "RAZORCAP":
                    status, task_id = await create_razorcap_task(
                        async_session=self.async_session,
                        api_key=API_KEY_RAZORCAP,
                        sitekey=SITEKEY,
                        proxy=self.proxy,
                        rqdata=captcha_rqdata
                    )
                if not status:
                    return False, f'[{self.acc_index}] | {self.discord_token} | {task_id}'
                logger.info(f"[{self.acc_index}] | {self.discord_token} | Task created, ID: {task_id}. Waiting for captcha answer...")

                if CAPTCHA_SERVICE == "CAPTCHA24":
                    status, g_recaptcha_response = await get_24captcha_task_result(self.async_session, API_KEY_24_CAPTCHA, task_id)
                elif CAPTCHA_SERVICE == "BESTCAPTCHA":
                    status, g_recaptcha_response = await get_bestcaptcha_task_result(self.async_session, API_KEY_BESTCAPTCHA, task_id)
                elif CAPTCHA_SERVICE == "RAZORCAP":
                    status, g_recaptcha_response = await get_razorcap_task_result(self.async_session, task_id)
                if not status:
    ...
    async def start_accept_discord_invite(self):
        for num in range(1, NUMBER_OF_ATTEMPTS + 1):
            try:
                ...
                logger.info(f'[{self.acc_index}] | {self.discord_token} | cон {sleep_time} секунд после успешной авторизации')
                ...
                if "Banned" in answer or 'Incorrect discord token' in answer:
                    logger.error(answer)
                    await self.close()
                    return False
